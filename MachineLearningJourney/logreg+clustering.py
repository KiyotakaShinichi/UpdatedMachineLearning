# logreg_with_clusters_first.py
# Clustering-first pipeline: KMeans -> add ClusterLabel as feature -> Logistic Regression
# Produces: CV per-fold accuracies, learning curve, confusion matrix, ROC curve,
# silhouette+elbow validation, PCA visualization, and CSV exports.
...
FINAL_CSV = r"C:/Users/L/Downloads/final_results_clusters_first.csv"
CLUSTER_PROFILE_CSV = r"C:/Users/L/Downloads/patient_cluster_profiles_clusters_first.csv"
CENTROIDS_CSV = r"C:/Users/L/Downloads/cluster_centroids_clusters_first.csv"
...
# ---------------------------
# 2) Scale features (original features only) and run KMeans first
# ---------------------------
print("\n🔍 Scaling original features and running KMeans (clustering first)...")
scaler_feat = StandardScaler()
X_features = df[selected_features].values
X_scaled_features = scaler_feat.fit_transform(X_features)

# choose sensible K range (avoid K >= n_samples)
n_samples = X_scaled_features.shape[0]
...
for k in K_range:
    ...
    labels = km.fit_predict(X_scaled_features)
    try:
        sil = silhouette_score(X_scaled_features, labels)
    except Exception:
        sil = -1.0
...
# Fit final KMeans on features
kmeans = KMeans(n_clusters=best_k, random_state=42, n_init=10)
cluster_labels = kmeans.fit_predict(X_scaled_features)
df['ClusterLabel'] = cluster_labels
print(f"✅ ClusterLabel added to df (k={best_k}).")

# Centroids in original feature scale
centroids_scaled = kmeans.cluster_centers_
centroids_unscaled = scaler_feat.inverse_transform(centroids_scaled)
...
# ---------------------------
# 3) Prepare dataset for classification (include ClusterLabel as a feature)
# ---------------------------
# We'll scale everything (including ClusterLabel) before train/test split so LR sees standardized inputs.
print("\n🔧 Preparing features for classification (including ClusterLabel)...")
X_all = df[selected_features + ['ClusterLabel']].copy()
y = df['DiabetesStatus'].copy()

scaler_all = StandardScaler()
X_all_scaled = scaler_all.fit_transform(X_all.values)  # includes cluster column scaled

# ---------------------------
# 4) Train/test split using indices so we can map back easily later
# ---------------------------
print("\n✂️ Splitting into train/test (80/20 stratified)...")
idx = np.arange(len(df))
train_idx, test_idx, y_train, y_test = train_test_split(idx, y, test_size=0.2, stratify=y, random_state=42)
...
# Also keep unscaled test original-feature DataFrame for CSV
X_test_unscaled = df.loc[test_idx, selected_features].reset_index(drop=True)
test_clusters = df.loc[test_idx, 'ClusterLabel'].values
...
# ---------------------------
# 6) Cross-validation fold accuracies (per-fold) - interactive plot
# ---------------------------
...
for train_ix, val_ix in skf.split(X_train_scaled, y_train):
    ...
    lr_clone.fit(X_train_scaled[train_ix], y_train.iloc[train_ix])
    preds = lr_clone.predict(X_train_scaled[val_ix])
...
fig_lc.update_layout(title='Learning Curve - Logistic Regression', xaxis_title='Number of training examples', yaxis_title='Accuracy')
fig_lc.show()

# ---------------------------
# 8) Final evaluation on test set & metrics table
# ---------------------------
...
print("\n📊 Test set metrics (Logistic Regression with Cluster feature):")
...
# ---------------------------
# 9) PCA visualization (interactive)
# ---------------------------
pca = PCA(n_components=2)
proj = pca.fit_transform(X_all_scaled)  # project scaled features including cluster
df['PCA1'] = proj[:,0]
df['PCA2'] = proj[:,1]

# probability for full dataset (LR predict_proba on scaled all features)
full_proba = lr.predict_proba(X_all_scaled)[:,1]
...
# ---------------------------
# 10) Final CSV: test rows with predictions, prob, cluster
# ---------------------------
